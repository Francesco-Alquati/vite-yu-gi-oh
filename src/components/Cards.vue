    ...
    <div class="container">
        <div class="row">
            <div class="col-3 my-3" v-for="card in store.cardsList" :key="card.id">
                <div class="bg-yugi d-flex flex-column align-items-center justify-content-center text-white text-center">
                    <img :src="card.card_images[0].image_url" />
                    <h6 class="mt-3">{{ card.name }}</h6>
                    <div class="mt-3">
                        <span v-if="card.archetype">{{ card.archetype }}</span>
                        <div v-else>unchnown</div>
                    </div>
                </div>
            </div>
        </div>
